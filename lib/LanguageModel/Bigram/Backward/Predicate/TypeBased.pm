package LanguageModel::Bigram::Backward::Predicate::TypeBased;
...
use base qw/LanguageModel::Bigram::Backward::TypeBased LanguageModel::Bigram::Backward::Predicate/;
...
sub addSentence {
    ...
    $self->{bigram}->add($prev, $pred); # reverse
}
...
sub removeSentence {
    ...
    my $pred = $sentence->{pred} || '$';
    $self->{bigram}->remove($prev, $pred); # reverse
}
...
sub sampleOneBoundary {
    ...
    my $newSegType = $self->LanguageModel::Bigram::Backward::Predicate::sampleBoundary($region, $iter, $anneal);
